...
function ActionBar() {
  ...
  const reviveBalance = Number(tokenBalances['REVIVE'] ?? 0);
  const attackBalance = Number(tokenBalances['ATTACK'] ?? 0);
  const extraLifeBalance = Number(tokenBalances['EXTRA LIFE'] ?? 0);
  const poisonBalance = Number(tokenBalances['POISON'] ?? 0);
