  ...
  const fetchPotionQuote = useCallback(
    async (potionId: string, tokenSymbol: string, quantity: number) => {
      ...
      try {
        ...
        if (quote) {
          const rawAmount = (quote.totalDisplay * -1) / Math.pow(10, selectedTokenData.decimals || 18);
          if (rawAmount === 0) {
            setTokenQuotes(prev => ({
              ...prev,
  ...
  const fetchSellQuote = useCallback(
    async (potionId: string, tokenSymbol: string, quantity: number) => {
      ...
      try {
        ...
        if (quote) {
          const rawAmount = quote.totalDisplay / Math.pow(10, receiveTokenData.decimals || 18);
          if (rawAmount === 0) {
            setTokenQuotes(prev => ({
              ...prev,
              [potionId]: { amount: '', loading: false, error: 'Insufficient liquidity' }
