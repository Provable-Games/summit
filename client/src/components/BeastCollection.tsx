...
function BeastCollection() {
  ...
  const collectionWithCombat = useMemo(() => {
    if (summit && collection.length > 0) {
      let filtered = collection.map((beast: Beast) => {
        ...
        newBeast.combat = calculateBattleResult(
          newBeast,
          summit,
      ...
      if (typeFilter === "strong") {
        filtered = filtered.filter((beast) =>
          isStrongAgainst(beast.type, summit.beast.type)
        );
      }
      ...
      if (hideDeadBeasts) {
        filtered = filtered.filter(
          (beast) => beast.current_health > 0 && !isBeastLocked(beast)
        );
      }
      ...
      if (hideTop5000) {
        filtered = filtered.filter(
          (beast) => !isBeastInTop5000(beast, top5000Cutoff)
        );
      }
  ...
  const selectBeast = useCallback(
    (beast: Beast) => {
      ...
      if (
        selectedBeasts.find(
          (prevBeast) => prevBeast.token_id === beast.token_id
        )
      ) {
        setSelectedBeasts((prev: Beast[]) =>
          prev.filter((prevBeast) => prevBeast.token_id !== beast.token_id)
        );
      } else {
  ...
  const handleHoverEnter = useCallback(
    (event: React.MouseEvent<HTMLElement>, beast: Beast) => {
        ...
        <Box sx={styles.emptyState}>
          <Box sx={styles.connectWalletContent}>
            ...
            <Box sx={styles.connectTextContainer}>
              <Typography sx={styles.emptyStateTitle}>GET STARTED</Typography>
        ...
        <Box sx={styles.emptyState}>
          <Box sx={styles.noBeastsContent}>
            ...
            <Box sx={styles.singleBeastShowcase}>
              <img
                src="/images/beasts/wolf.png"
                alt=""
                style={styles.sadBeastImage}
              />
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={styles.emptyState}>
          <Box sx={styles.loadingStateContent}>
            ...
            <Box sx={styles.loadingBeastContainer}>
              ...
              <img
                src="/images/beasts/warlock.png"
                alt=""
                style={styles.loadingBeastImage}
              />
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={styles.beastGridContainer}>
          ...
          <Box sx={styles.leftSideContainer}>
            ...
            <Box sx={styles.utilityButtonsContainer}>
              <Tooltip
                placement="top"
                title={<Box sx={styles.tooltipContent}>Filter & Sort</Box>}
              >
                <Box
                  sx={[
                    styles.utilityButton,
                    filterExpanded && styles.selectedItem,
                  ]}
                  onClick={() => setFilterExpanded(!filterExpanded)}
                >
                  <FilterListIcon
                    sx={{ color: gameColors.brightGreen, fontSize: "20px" }}
                  />
                </Box>
              </Tooltip>
              ...
              >
                <Box
                  sx={[
                    styles.utilityButton,
                    maxBeastsSelected && styles.selectedItem,
                  ]}
                  onClick={() => selectAllBeasts()}
                >
                  <LibraryAddCheckIcon
                    sx={{ color: gameColors.brightGreen, fontSize: "20px" }}
                  />
              ...
              {filterExpanded && (
                ...
                >
                  <Box sx={styles.filterPanel}>
                    {/* Sort Dropdown */}
                    <Box>
                      <Typography sx={styles.filterHeadline}>
                        SORT BEASTS BY
                      </Typography>
                      <Box
                      ...
                      >
                        <Box sx={styles.sortDropdownCurrent}>
                          <Box sx={styles.sortOptionIcon}>
                            ...
                            {sortMethod === "health" && (
                              <FavoriteIcon
                                sx={{ fontSize: "12px", color: gameColors.red }}
                              />
                            )}
                            {sortMethod === "rewards earned" && (
                              <img
                              ...
                              {(
                                ...
                                <Box
                                  ...
                                  sx={[
                                    ...
                                    sortMethod === method &&
                                      styles.sortDropdownOptionActive,
                                  ]}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                ...
                                >
                                  ...
                                  <Typography
                                    sx={styles.sortDropdownOptionText}
                                  >
                                    {method === "recommended"
                                      ? "Recommended"
                    ...
                    <Box mb={0.5}>
                      ...
                      <Box sx={styles.filterButtons}>
                        {(["all", "strong"] as BeastTypeFilter[]).map(
                          (type) => (
                            <Box
                              key={type}
                              sx={[
                    ...
                    <Box>
                      ...
                      <Box sx={styles.compactToggles}>
                        <Box
                          sx={[
                            ...
                            hideDeadBeasts && styles.compactToggleActive,
                          ]}
                          onClick={() => hideDead(!hideDeadBeasts)}
                        >
                          <Typography sx={styles.compactToggleText}>
                            Hide Dead
                          </Typography>
                        </Box>
                        <Box
                          sx={[
                            styles.compactToggle,
                            nameMatchFilter && styles.compactToggleActive,
                          ]}
                      ...
                      <Box sx={styles.compactToggles} mt={0.5}>
                        <Box
                          sx={[
                            styles.compactToggle,
                            hideTop5000 && styles.compactToggleActive,
                          ]}
            ...
            >
              ...
              >
                {virtualizer.getVirtualItems().map((virtualItem) => {
                  ...
                  const isSelected = selectedBeasts.some(
                    (b) => b.token_id === beast.token_id
                  );
