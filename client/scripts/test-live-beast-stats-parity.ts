...
function runVector(name: string, stats: LiveBeastStats): void {
...
const MAX_VECTOR: LiveBeastStats = {
  ...
  bonus_xp: 0x7fff,
  attack_streak: 0xf,
  // Safe integer limit for client number representation.
...
runVector("cross-layer parity", CROSS_LAYER_PARITY_EXPECTED);
runVector("all-zero values", ZERO_VECTOR);
runVector("max values", MAX_VECTOR);

for (const { name, key } of FLAG_CASES) {
  const vector: LiveBeastStats = { ...ZERO_VECTOR, [key]: true };
  runVector(`flag isolation: ${name}`, vector);
}
...
console.log("Client LiveBeastStats packing/unpacking tests passed.");
