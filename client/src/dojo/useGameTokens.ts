  ...
  const getBeastCollection = async (accountAddress: string, cachedCollection: Beast[]) => {
    ...
    let beasts: Beast[] = integerIds.map((numericTokenId) => {
      ...
      let beast: any = {
        ...
        current_health: stats["live_stats.current_health"] || 0,
        ...
        last_death_timestamp: Math.max(cachedBeast?.last_death_timestamp || 0, parseInt(stats["live_stats.last_death_timestamp"], 16) || 0),
