export interface SummitConfig {
  ...
  starknetWsUrl?: string;
  startingBlock: string;
  databaseUrl?: string;
  rpcUrl?: string;
}

export const summitRuntimeConfig: SummitConfig = {
  ...
  // Mainnet DNA stream URL for the legacy Apibara path.
  streamUrl: process.env.STREAM_URL,
  // HTTP and WebSocket Starknet JSON-RPC URLs for the accepted-L2 RPC path.
  starknetRpcUrl: process.env.STARKNET_RPC_URL,
  starknetWsUrl: process.env.STARKNET_WS_URL,
  // Starting block - use earliest block needed for Dojo events.
  startingBlock: process.env.STARTING_BLOCK ?? "7077225",
  // PostgreSQL connection string.
  databaseUrl: process.env.DATABASE_URL,
  // RPC URL for metadata calls. Defaults to the same HTTP RPC endpoint used for indexing.
  rpcUrl: process.env.STARKNET_METADATA_RPC_URL ?? process.env.STARKNET_RPC_URL,
};
